  ...
  class TenantServiceImpl implements TenantService {
	...
	public Optional<TenantTo> findTenant(int tenantId){
		if(tenantId <1) 
		 ...
		 return tenantRepository.findById(tenantId).filter(r -> !r.isDeleted()).map(r -> converter.convert(r,TenantTo.class));
	}
	...
	public Set<UserTo> findUsers(int tenantId){
		return tenantRepository.findById(tenantId).get().getUsers().stream().map(e->converter.convert(e, UserTo.class)).collect(Collectors.toSet());
	}
	 ...
	 */
	@Autowired
	public TenantServiceImpl(ConversionService converter, EmailTemplateRepository emailtemplateRepository, ProgramRepository programRepository, UserRepository userRepository, InstanceRepository instanceRepository, OrganizationRepository organizationRepository, NumberSequenceRuleRepository numbersequenceruleRepository, CustomerRepository customerRepository, PersonRepository personRepository, TenantRepository tenantRepository){
